...
# Class is imported
from population.Pages.extract_population_page import WorldCounts
import keyboard #Keyboard is imported to check the keyboard activity

# Marked this as Sanity test
@pytest.mark.sanity
#To test the population class and its methods
def test_world_counts(set_up):
    driver=set_up
    world_count=WorldCounts(driver) # Creating object for class
    counter = world_count.population_counts() #Calling the methods
    world_count.dismiss_cookies()
    print("\nWorld Population")
    while True:
        try:
            print(counter.text)                # To print the population counts on console
